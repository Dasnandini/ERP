...
function LoginForm() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      // Refresh Auth Context and check company status
      await refreshUser();

      // Check current user details via service
  ...
  return (
    <div className="w-full max-w-md bg-slate-900/90 border border-slate-800 backdrop-blur-xl rounded-2xl p-8 shadow-2xl shadow-indigo-950/20 relative z-10">
      ...
      <form className="flex flex-col gap-4" onSubmit={handleSubmit} noValidate>
        ...
        <button
          type="submit"
          className="w-full py-2.5 px-4 bg-indigo-600 hover:bg-indigo-500 text-white font-medium rounded-xl text-sm transition-all shadow-lg shadow-indigo-600/25 active:scale-[0.99] disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 mt-2"
          id="login-submit"
          disabled={loading}
        >
          {loading && (
